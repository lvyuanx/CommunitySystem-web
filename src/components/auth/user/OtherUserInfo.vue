  ...
  <div class="box exceptionInfoBox">
    <van-swipe-cell class="box cellBox">
      <van-row class="rowBox">
        <van-col span="8" class="imageBox" :style="{'background-image': 'url(' + avatarUrl + ')'}">
        </van-col>
        <van-col span="16 infoBox">
          ...
          <van-row class="lineBox">
            ...
            <van-col span="24">
              <van-button v-if="userInfo.householdId != null" plain type="info" @click.stop="showPopup(userInfo.userId)">{{userInfo.periodNumber}} / {{userInfo.buildingNumber}} / {{userInfo.unitNumber}} / {{userInfo.layerNumber}} / {{userInfo.householdNumber}}</van-button>
              <van-button v-if="userInfo.householdId == null" plain type="info" icon="plus" style="width:100%" @click="showPopup(userInfo.userId)">添加地址</van-button>
            </van-col>
          </van-row>
        </van-col>
      </van-row>
...
export default {
  ...
  methods: {
    ...
    showPopup(userId) { // 显示地址选择组件
      let params = {
        userId: userId,
        isShow: true
      }
      this.$bus.$emit("showSelectAddress", params) //使用自定义事件传递数据
    },
    changeCode() { // 转码
