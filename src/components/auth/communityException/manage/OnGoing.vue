<template>
  <div class="box exceptionInfoBox">
    <van-swipe-cell class="box cellBox">
      ...
      <template #right>
        <van-button square type="danger" class="cell-button" @click="endExec()" text="结束" />
      </template>
    </van-swipe-cell>
    ...
    <van-dialog v-model="showEndTab" title="处理结果" show-cancel-button :before-close="beforeClose" @confirm="confirm(exceptionInfo.id)">
      ...
      <van-field name="radio" label="是否转码">
        <template #input>
          <van-radio-group v-model="radio" direction="horizontal">
            <van-radio icon-size="15px" shape="square" name="">否</van-radio>
            <van-radio icon-size="15px" shape="square" name="yellow">黄码</van-radio>
            <van-radio icon-size="15px" shape="square" name="red">红码</van-radio>
          </van-radio-group>
        </template>
      </van-field>
    </van-dialog>
